...
{
    /**
     * @Route("/user/edit/profil/{slug}", name="user_edit_profile", methods={"GET", "POST"})
     */
    public function editUserProfile(EntityManagerInterface $entityManager, Request $request, SluggerInterface $slugger): Response
    {
        ...
        $user = $this->getUser();
            ...
            // if no user is authenticated, we create a new one
            if ( !$user)
            {
        ...
        return $this->renderForm('front/form/user.html.twig', compact('form'));
    }
    ...
    public function showPlacePanel(EventRepository $eventRepository, UserRepository $userRepository,  string $slug): Response
    {
        ...
        return $this->render('front/main/user.html.twig', compact('user'));
    }
